    ...
    public partial class HomeController : BaseController
    {
        ...
        public IActionResult ChargeTag(string Id, ChargeTagViewModel ctvm)
        {
            try
            {
                ...
                ViewBag.TagGroups = DbContext.TagGroups.OrderBy(g => g.Name).ToList();
                ...
                {
                    // Manual parsing of ExpiryDate to handle culture mismatches from the DatePicker
                    if (!ctvm.ExpiryDate.HasValue && !string.IsNullOrEmpty(Request.Form["ExpiryDate"]))
                    {
                        if (DateTime.TryParseExact(Request.Form["ExpiryDate"], "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime parsed))
                        {
                            ctvm.ExpiryDate = parsed;
                        }
                        else if (DateTime.TryParse(Request.Form["ExpiryDate"], out DateTime parsedFallback))
                        {
                            ctvm.ExpiryDate = parsedFallback;
                        }
                    }
