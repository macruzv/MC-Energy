    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<IActionResult> Login(UserModel userModel, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                Logger.LogInformation("Login Attempt: User '{0}' - ReturnUrl: '{1}'", userModel.Username, returnUrl);
                try
                {
                    bool success = await UserManager.SignIn(this.HttpContext, userModel, false);
                    if (success)
                    {
                        Logger.LogInformation("User '{0}' logged in successfully. Redirecting to: {1}", userModel.Username, returnUrl ?? "Home");
                        return RedirectToLocal(returnUrl);
                    }
                    else
                    {
                        Logger.LogWarning("Invalid login attempt: User '{0}'", userModel.Username);
                ...
                {
                    Logger.LogError(ex, "Login Exception for user '{0}': {1}", userModel.Username, ex.Message);
                    ModelState.AddModelError(string.Empty, $"Error de sistema: {ex.Message}");
                    return View(userModel);
                }
            }
