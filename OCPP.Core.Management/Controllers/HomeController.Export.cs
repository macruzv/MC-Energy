...
namespace OCPP.Core.Management.Controllers
{
    public partial class HomeController : BaseController
    {
        ...
        private TransactionListViewModel LoadTransactionListViewModel(string Id, string ConnectorId)
        {
            ...
            {
                tlvm.CurrentConnectorName = "All_Connectors";
            }
        ...
        private XLWorkbook CreateSpreadsheet(TransactionListViewModel tlvm)
        {
            ...
            worksheet.Cell(1, 1).Value = (_localizer?["Connector"] ?? "Connector").ToString();
            worksheet.Cell(1, 2).Value = (_localizer?["StartTime"] ?? "Start Time").ToString();
            worksheet.Cell(1, 3).Value = (_localizer?["StartTag"] ?? "Start Tag").ToString();
            worksheet.Cell(1, 4).Value = (_localizer?["MeterStart"] ?? "Meter Start").ToString();
            worksheet.Cell(1, 5).Value = (_localizer?["StopTime"] ?? "Stop Time").ToString();
            worksheet.Cell(1, 6).Value = (_localizer?["StopTag"] ?? "Stop Tag").ToString();
            worksheet.Cell(1, 7).Value = (_localizer?["MeterStop"] ?? "Meter Stop").ToString();
            worksheet.Cell(1, 8).Value = (_localizer?["ChargeSum"] ?? "Charge Sum").ToString();
