...
namespace OCPP.Core.Management.Controllers
{
    public partial class HomeController : BaseController
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(serverApiUrl))
                {
                    ...
                    IQueryable<ChargePoint> cpQuery = DbContext.ChargePoints;
                    ...
                    if (!User.IsInRole(Constants.AdminRoleName))
                    {
                        ...
                        {
                            // Safety fallback: if not admin and no valid userId found, show nothing
                            cpQuery = cpQuery.Where(cp => false);
                        }
                    }

                    List<ChargePoint> dbChargePoints = cpQuery.ToList();
                        ...
                        if (connectorStatusViewList != null)
                        {
                            foreach (ConnectorStatusView connStatus in connectorStatusViewList)
                            {
                                if (string.Equals(cp.ChargePointId, connStatus.ChargePointId, StringComparison.InvariantCultureIgnoreCase))
                                {
                                    ...
                                    // CRITICAL: If the charge point is not online, the status must reflect that
                                    // regardless of what the database says about transactions.
                                    if (!cpovm.Online)
                                    {
                                        cpovm.ConnectorStatus = ConnectorStatusEnum.Unavailable; // Use Unavailable (Offline)
                                    }
                ...
                // Load 5 recent transactions for dashboard
                var recentTransactionsQuery = (from t in DbContext.Transactions
                                                     ...
                                                     where (t.StopTime == null || t.StartTime >= DateTime.UtcNow.AddHours(-24))
                                                     select new { t, startCT });
                ...
                if (!User.IsInRole(Constants.AdminRoleName))
                {
                    ...
                    if (int.TryParse(userIdStr, out int userId))
                    {
                        ...
                        recentTransactionsQuery = recentTransactionsQuery.Where(x => assignedIds.Contains(x.t.ChargePointId));
                    }
                    else
                    {
                        recentTransactionsQuery = recentTransactionsQuery.Where(x => false);
                    }
                }

                overviewModel.RecentTransactions = recentTransactionsQuery
                                                     .Select(x => new TransactionExtended
                                                     {
                ...
                // Calculate Real Energy Stats
                var allTransactionsQuery = DbContext.Transactions
                    .Where(t => t.MeterStop != null);
                ...
                if (!User.IsInRole(Constants.AdminRoleName))
                {
                    ...
                    if (int.TryParse(userIdStr, out int userId))
                    {
                        ...
                        allTransactionsQuery = allTransactionsQuery.Where(t => assignedIds.Contains(t.ChargePointId));
                    }
                    else
                    {
                        allTransactionsQuery = allTransactionsQuery.Where(t => false);
                    }
                }

                var allTransactions = allTransactionsQuery
                    .Select(t => new { t.StartTime, t.MeterStart, t.MeterStop })
                    .AsNoTracking()
                    .ToList();

                overviewModel.TotalEnergy = allTransactions.Sum(t => t.MeterStop.Value - t.MeterStart);

                DateTime todayStart = DateTime.UtcNow.Date;
                DateTime yesterdayStart = todayStart.AddDays(-1);

                double energyToday = allTransactions
                    .Where(t => t.StartTime >= todayStart)
                    .Sum(t => t.MeterStop.Value - t.MeterStart);

                double energyYesterday = allTransactions
                    .Where(t => t.StartTime >= yesterdayStart && t.StartTime < todayStart)
                    .Sum(t => t.MeterStop.Value - t.MeterStart);

                if (energyYesterday > 0)
                {
                    overviewModel.EnergyTrendPercent = Math.Round(((energyToday - energyYesterday) / energyYesterday) * 100, 1);
                    overviewModel.IsTrendUp = energyToday >= energyYesterday;
                }
                else
                {
                    overviewModel.IsTrendUp = true;
                }

                // Energy Graph Data (Last 24h)
                var last24hStart = DateTime.UtcNow.AddHours(-24);
                var historyData = allTransactions
                    .Where(t => t.StartTime >= last24hStart)
                    .GroupBy(t => new { t.StartTime.Year, t.StartTime.Month, t.StartTime.Day, t.StartTime.Hour })
                    .Select(g => new EnergyDataPoint
                    {
                        Time = new DateTime(g.Key.Year, g.Key.Month, g.Key.Day, g.Key.Hour, 0, 0, DateTimeKind.Utc),
                        Value = g.Sum(t => t.MeterStop.Value - t.MeterStart)
                    })
                    .OrderBy(x => x.Time)
                    .ToList();

                // Fill missing hours with 0
                overviewModel.EnergyHistory = new List<EnergyDataPoint>();
                for (int i = 0; i <= 24; i++)
                {
                    var timePoint = last24hStart.AddHours(i);
                    timePoint = new DateTime(timePoint.Year, timePoint.Month, timePoint.Day, timePoint.Hour, 0, 0, DateTimeKind.Utc);
                    
                    var existing = historyData.FirstOrDefault(x => x.Time == timePoint);
                    overviewModel.EnergyHistory.Add(new EnergyDataPoint 
                    { 
                        Time = timePoint, 
        ...
        {
            return View(new List<ChargePointsOverviewViewModel>());
        }
    }
}
