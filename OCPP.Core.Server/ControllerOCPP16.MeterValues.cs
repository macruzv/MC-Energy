...
namespace OCPP.Core.Server
{
    public partial class ControllerOCPP16
    {
        public string HandleMeterValues(OCPPMessage msgIn, OCPPMessage msgOut)
        {
                ...
                if (ChargePointStatus != null)
                {
                    ...
                    foreach (MeterValue meterValue in meterValueRequest.MeterValue)
                    {
                        foreach (SampledValue sampleValue in meterValue.SampledValue)
                        {
                            ...
                            if (sampleValue.Measurand == SampledValueMeasurand.Power_Active_Import)
                            {
                                ...
                                {
                                    Logger.LogError("{ControllerName} => MeterValues => Charging: invalid value '{0}' (Unit={1})", GetType().Name, sampleValue.Value, sampleValue.Unit);
                                }
                            }
                                    ...
                                    sampleValue.Measurand == null)
                            {
                                ...
                                {
                                    Logger.LogError("{ControllerName} => MeterValues => Value: invalid value '{0}' (Unit={1})", GetType().Name, sampleValue.Value, sampleValue.Unit);
                                }
                            }
                    ...
                    if (connectorId > 0)
                    {
                        ...
                        if (meterKWH >= 0)
                        {
                            ...
                            // PERSISTENCE: Also update the active transaction record if it exists
                            try
                            {
                            ...
                            {
                                Logger.LogWarning("MeterValues => Error persisting intermediate meter: {0}", ex.Message);
                            }
                        }
                        ...
                        if (currentChargeKW >= 0 || meterKWH >= 0 || stateOfCharge >= 0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Logger.LogError("{ControllerName} => MeterValues => Error adding new OnlineConnectorStatus for ChargePoint={0} / Connector={1} / Values: {2}", GetType().Name, ChargePointStatus?.Id, connectorId, msgMeterValue);
                                }
                            }
                        }
                    }
            ...
            {
                Logger.LogError(exp, "{ControllerName} => MeterValues => Exception: {0}", GetType().Name, exp.Message);
