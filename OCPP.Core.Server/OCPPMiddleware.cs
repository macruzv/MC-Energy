...
namespace OCPP.Core.Server
{
    public partial class OCPPMiddleware
    {
        ...
        // Dictionary for temporary customer info storage
        private static ConcurrentDictionary<(string cpId, int cnId), CustomerInfo> _pendingCustomerData = new ConcurrentDictionary<(string cpId, int cnId), CustomerInfo>();
        ...
        public static CustomerInfo GetPendingCustomerData(string cpId, int cnId)
        {
            CustomerInfo info = null;
            if (_pendingCustomerData.TryRemove((cpId, cnId), out info))
            {
                // check if data is not too old (e.g. 5 minutes)
                if (info.Timestamp > DateTime.Now.AddMinutes(-5))
                {
                    return info;
                }
            }
        ...
        public static List<(string, int)> GetAllPendingCustomerKeys()
        {
            return _pendingCustomerData.Keys.ToList();
        }
                    ...
                    if (context.WebSockets.IsWebSocketRequest)
                    {
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                // Use indexer to add or update the entry for the charge point. This prevents ArgumentException on reconnect.
                        ...
                        if (!string.IsNullOrEmpty(urlChargePointId))
                        {
                            try
                            {
                                ...
                                if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                                {
                                    ...
                                    {
                                        // OCPP V1.6
                                        await RemoteStopTransaction16(status, context, dbContext, urlParam);
                                    }
                                    else
                                    {
                                        _logger.LogError("OCPPMiddleware RemoteStop => Not implemented for: {0}", status.Protocol);
                                ...
                                {
                                    // Chargepoint offline
                                    _logger.LogError("OCPPMiddleware RemoteStop => Chargepoint offline: {0}", urlChargePointId);
                                    context.Response.StatusCode = (int)HttpStatusCode.NotFound;
                                }
                            }
                            catch (Exception exp)
                            {
                                _logger.LogError(exp, "OCPPMiddleware RemoteStop => Error: {0}", exp.Message);
                                context.Response.StatusCode = (int)HttpStatusCode.InternalServerError;
                            }
                        }
                    }
                    else if (cmd == "RemoteStart")
                    {
                        if (!string.IsNullOrEmpty(urlChargePointId))
                        {
                            try
                            {
                                ...
                                if (_chargePointStatusDict.TryGetValue(urlChargePointId, out status))
                                {
                                    ...
                                    if (status.Protocol == Protocol_OCPP16)
                                    {
                                        // Capture customer data from query string
                                    ...
                                    {
                                        _logger.LogError("OCPPMiddleware RemoteStart => Not implemented for: {0}", status.Protocol);
                                        context.Response.StatusCode = (int)HttpStatusCode.NotImplemented;
                                    }
                                }
                                else
                                {
                                    // Chargepoint offline
                                    _logger.LogError("OCPPMiddleware RemoteStart => Chargepoint offline: {0}", urlChargePointId);
                                    context.Response.StatusCode = (int)HttpStatusCode.NotFound;
                                }
                            }
                            catch (Exception exp)
                            {
                                _logger.LogError(exp, "OCPPMiddleware RemoteStart => Error: {0}", exp.Message);
                                context.Response.StatusCode = (int)HttpStatusCode.InternalServerError;
                            }
                        }
                    }
