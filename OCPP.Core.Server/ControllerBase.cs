...
namespace OCPP.Core.Server
{
    public partial class ControllerBase
    {
        ...
        public ControllerBase(IConfiguration config, ILoggerFactory loggerFactory, ChargePointStatus chargePointStatus, OCPPCoreContext dbContext)
        {
            ...
            {
                Logger.LogError("{ControllerName} => New ControllerBase => empty chargepoint status", GetType().Name);
            }
            DbContext = dbContext;
        }
        ...
        protected T DeserializeMessage<T>(OCPPMessage msg)
        {
            ...
            if (!string.IsNullOrEmpty(schemaJson))
            {
                ...
                if (messages.Count > 0)
                {
                    ...
                    {
                        Logger.LogWarning("{ControllerName} => DeserializeMessage {0} => Validation error: {1}", GetType().Name, msg.Action, err);
                    }
        ...
        protected bool UpdateConnectorStatus(int connectorId, string status, DateTimeOffset? statusTime, double? meter, DateTimeOffset? meterTime)
        {
            ...
            {
                Logger.LogWarning("UpdateConnectorStatus => Concurrency exception (item potentially deleted): {0}", dex.Message);
                foreach (var entry in dex.Entries)
                {
                    if (entry.Entity is ConnectorStatus)
                    {
            ...
            {
                Logger.LogError(exp, "{ControllerName} => UpdateConnectorStatus => Exception writing connector status (ID={0} / Connector={1}): {2}", GetType().Name, ChargePointStatus?.Id, connectorId, exp.Message);
            }
