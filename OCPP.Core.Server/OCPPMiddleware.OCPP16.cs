...
namespace OCPP.Core.Server
{
    public partial class OCPPMiddleware
    {
        ...
        /// <summary>
        /// Sends a RemoteStopTransaction-Request to the chargepoint
        /// </summary>
        private async Task RemoteStopTransaction16(ChargePointStatus chargePointStatus, HttpContext apiCallerContext, OCPPCoreContext dbContext, string urlParam)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            ControllerOCPP16 controller16 = new ControllerOCPP16(_configuration, _logFactory, chargePointStatus, dbContext);

            Messages_OCPP16.RemoteStopTransactionRequest remoteStopRequest = new Messages_OCPP16.RemoteStopTransactionRequest();
            if (int.TryParse(urlParam, out int transactionId))
            {
                remoteStopRequest.TransactionId = transactionId;
            }

            logger.LogTrace("OCPPMiddleware.OCPP16 => RemoteStopTransaction16: ChargePoint='{0}' / TransactionId={1}", chargePointStatus.Id, remoteStopRequest.TransactionId);
        ...
        /// <summary>
        /// Sends a RemoteStartTransaction-Request to the chargepoint
        /// </summary>
        private async Task RemoteStartTransaction16(ChargePointStatus chargePointStatus, HttpContext apiCallerContext, OCPPCoreContext dbContext, string urlConnectorId, string urlParam)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            ControllerOCPP16 controller16 = new ControllerOCPP16(_configuration, _logFactory, chargePointStatus, dbContext);

            Messages_OCPP16.RemoteStartTransactionRequest remoteStartRequest = new Messages_OCPP16.RemoteStartTransactionRequest();
            if (int.TryParse(urlConnectorId, out int connectorId))
            {
                remoteStartRequest.ConnectorId = connectorId;
            }
            remoteStartRequest.IdTag = urlParam;

            logger.LogTrace("OCPPMiddleware.OCPP16 => RemoteStartTransaction16: ChargePoint='{0}' / ConnectorId={1} / IdTag={2}", chargePointStatus.Id, remoteStartRequest.ConnectorId, remoteStartRequest.IdTag);
