...
namespace OCPP.Core.Server
{
    public partial class ControllerOCPP20
    {
        public string HandleTransactionEvent(OCPPMessage msgIn, OCPPMessage msgOut, OCPPMiddleware ocppMiddleware)
        {
            ...
            try
            {
                ...
                else if (transactionEventRequest.EventType == TransactionEventEnumType.Updated)
                {
                    try
                    {
                        ...
                        {
                            Logger.LogError("{ControllerName} => UpdateTransaction => Unknown or not matching transaction: uid='{0}' / chargepoint='{1}' / tag={2}", GetType().Name, transactionEventRequest.TransactionInfo?.TransactionId, ChargePointStatus?.Id, idTag);
                    ...
                    {
                        Logger.LogError(exp, "{ControllerName} => UpdateTransaction => Exception: {0}", GetType().Name, exp.Message);
                    }
                }
                    ...
                    try
                    {
                            ...
                            !transaction.StopTime.HasValue)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Logger.LogError(exp, "{ControllerName} => EndTransaction => Exception from external authorization: {0}", GetType().Name, exp.Message);
                                }
                                ...
                                else
                                {
                                    ...
                                    {
                                        Logger.LogError(exp, "{ControllerName} => EndTransaction => Exception reading charge tag ({0}): {1}", GetType().Name, idTag, exp.Message);
                                        transactionEventResponse.IdTokenInfo.Status = AuthorizationStatusEnumType.Invalid;
                                    }
                                }
                            }
                        ...
                        else
                        {
                            ...
                            Logger.LogError("{ControllerName} => EndTransaction => Unknown or not matching transaction: id={0} / chargepoint={1} / tag={2}", GetType().Name, transactionEventRequest.TransactionInfo.TransactionId, ChargePointStatus?.Id, idTag);
                        ...
                        if (transactionEventResponse.IdTokenInfo.Status == AuthorizationStatusEnumType.Accepted)
                        {
                            try
                            {
                                    ...
                                    !transaction.StopTime.HasValue)
                                {
                                    ...
                                    if (!transaction.StartTagId.Equals(idTag, StringComparison.InvariantCultureIgnoreCase))
                                    {
                                        ...
                                        {
                                            Logger.LogError("{ControllerName} => EndTransaction => Start-Tag not found: '{0}'", GetType().Name, transaction.StartTagId);
                                ...
                                {
                                    Logger.LogError("{ControllerName} => EndTransaction => Unknown or not matching transaction: uid='{0}' / chargepoint='{1}' / tag={2}", GetType().Name, transactionEventRequest.TransactionInfo?.TransactionId, ChargePointStatus?.Id, idTag);
                            ...
                            {
                                Logger.LogError(exp, "{ControllerName} => EndTransaction => Exception: {0}", GetType().Name, exp.Message);
                                transactionEventResponse.IdTokenInfo.Status = AuthorizationStatusEnumType.Invalid;
                            }
                        }
                        #endregion
            ...
            {
                Logger.LogError(exp, "{ControllerName} => TransactionEvent => Exception: {0}", GetType().Name, exp.Message);
