...
namespace OCPP.Core.Server
{
    public partial class ControllerOCPP20 : ControllerBase
    {
        ...
        public OCPPMessage ProcessRequest(OCPPMessage msgIn, OCPPMiddleware ocppMiddleware)
        {
            ...
            {
                Logger.LogError("{ControllerName} => Protocol error: wrong message type '{0}'", GetType().Name, msgIn.MessageType);
            ...
            if (!string.IsNullOrEmpty(errorCode))
            {
                ...
                Logger.LogDebug("{ControllerName} => Return error code messge: ErrorCode={0}", GetType().Name, errorCode);
            }
        ...
        private void WriteMessageLog(string chargePointId, int? connectorId, string message, string result, string errorCode)
        {
            try
            {
                ...
                if (dbMessageLog > 0 && !string.IsNullOrWhiteSpace(chargePointId))
                {
                    ...
                    if (doLog)
                    {
                        ...
                        // Truncate result if it's too long for the database column to prevent DbUpdateException
                        if (msgLog.Result?.Length > 255)
                        {
                            msgLog.Result = msgLog.Result.Substring(0, 255);
                        }
            ...
            {
                Logger.LogError(exp, "{ControllerName} => MessageLog => Error writing entry '{0}'", GetType().Name, message);
            }
        }
    }
}
