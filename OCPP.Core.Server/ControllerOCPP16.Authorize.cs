    ...
    public partial class ControllerOCPP16
    {
        public string HandleAuthorize(OCPPMessage msgIn, OCPPMessage msgOut, OCPPMiddleware ocppMiddleware)
        {
            ...
            try
            {
                ...
                {
                    idTag = "21222122";
                    Logger.LogInformation("Authorize => Empty tag detected, using default: {0}", idTag);
                }
                ...
                if (idTag == "21222122")
                {
                    ...
                    authorizeResponse.IdTagInfo.ExpiryDate = MaxExpiryDate;
                    Logger.LogInformation("Authorize => System tag '{0}' whitelisted.", idTag);
                ...
                {
                    Logger.LogError(exp, "{ControllerName} => Authorize => Exception from external authorization: {0}", GetType().Name, exp.Message);
                }
                ...
                else
                {
                    try
                    {
                        ...
                        if (ct != null)
                        {
                            ...
                            else if (ct.ExpiryDate.HasValue && ct.ExpiryDate.Value < DateTime.Now)
                            {
                                ...
                                Logger.LogInformation("Authorize => Tag '{0}' expired and was automatically blocked.", idTag);
                            }
                            else
                            {
                    ...
                    {
                        Logger.LogError(exp, "{ControllerName} => Authorize => Exception reading charge tag ({0}): {1}", GetType().Name, idTag, exp.Message);
                        authorizeResponse.IdTagInfo.Status = IdTagInfoStatus.Invalid;
                    }
                }
            ...
            {
                Logger.LogError(exp, "{ControllerName} => Authorize => Exception: {0}", GetType().Name, exp.Message);
